...
#region -- WebUI data.js export -----------------------------------------------
$webUiDir    = Join-Path $QuietMonitorBase 'WebUI'
$dataJsPath  = Join-Path $webUiDir 'data.js'
try {
    if (-not (Test-Path $webUiDir)) { New-Item -ItemType Directory -Path $webUiDir -Force | Out-Null }
    $latestScanWithHost = $latestScan | Select-Object -Property *
    Add-Member -InputObject $latestScanWithHost -NotePropertyName 'host' -NotePropertyValue $env:COMPUTERNAME -Force
    $dataJson  = $latestScanWithHost | ConvertTo-Json -Depth 5 -Compress
    $dataJs    = "// Generated by QuietMonitor - $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')`nwindow.SCAN_DATA = $dataJson;"
    [System.IO.File]::WriteAllText($dataJsPath, $dataJs, [System.Text.Encoding]::UTF8)
    Write-Host "  [+] WebUI data     : $dataJsPath" -ForegroundColor Cyan
} catch {
    Write-Warning "  WebUI data.js export failed: $($_.Exception.Message)"
}
#endregion
